    ...
    try {
      ...
      const movieDetailsPromises = movieIds.map(async (movieId) => {
        const response = await axios.get(`https://api.themoviedb.org/3/movie/${movieId}?language=en-US`, {
          params: {         //https://api.themoviedb.org/3/find/external_id?external_source=imdb_id
            api_key: apiKey,
          },
        });
        ...
        if (movieDetails.title && movieDetails.poster_path) {
          return {
            ...
            type: movieDetails.media_type || 'movie', // Default to 'movie' if media_type is not available
          };
        } else {
          ...
          <ul>
            {data.map((item, index) => (
              <li key={index}>
                ...
                {item.type === 'movie' && <p></p>}
                {item.type === 'tv' && <p>This is a TV show</p>}
              </li>
            ))}
          </ul>
        )}
