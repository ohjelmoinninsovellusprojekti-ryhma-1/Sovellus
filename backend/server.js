...
app.post('/api/add-rating', async (req, res) => {
    ...
    try {
      await connection.beginTransaction();
      await connection.query('INSERT INTO movie_reviews (user_id, movie_id, rating) VALUES (?, ?, ?) ON DUPLICATE KEY UPDATE rating = VALUES(rating)', [userId, movieId, rating]);
      await connection.commit();
  ...
  // Tämä pois alhaalta jos ei toimi
  app.post('/api/add-a-review', async (req, res) => {
    const { movieId, reviewText, username } = req.body;
    ...
    try {
      ...
      await connection.query(
        'INSERT INTO movie_reviews (movie_id, review_text, created_at, username) VALUES (?, ?, NOW(), ?)',
        [movieId, reviewText, username]
      );
